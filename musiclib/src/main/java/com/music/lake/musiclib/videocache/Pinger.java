...
import static com.music.lake.musiclib.videocache.Preconditions.checkArgument;
...
//    private static final Logger LOG = LoggerFactory.getLogger("Pinger");
    ...
    boolean ping(int maxAttempts, int startTimeout) {
        ...
        while (attempts < maxAttempts) {
            try {
                ...
                Logger.w("Error pinging server (attempt: " + attempts + ", timeout: " + timeout + "). ");
            } catch (InterruptedException | ExecutionException e) {
                Logger.e("Error pinging server due to unexpected error", e);
            }
            attempts++;
            timeout *= 2;
        }
        ...
        Logger.e(error, new ProxyCacheException(error));
        return false;
    }
    ...
    private boolean pingServer() throws ProxyCacheException {
        ...
        try {
            ...
            Logger.i("Ping response: `" + new String(response) + "`, pinged? " + pingOk);
            return pingOk;
        } catch (ProxyCacheException e) {
            Logger.e("Error reading ping response", e);
            return false;
        } finally {
            source.close();
        }
